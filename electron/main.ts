...
async function wavToMp3(wavPath: string, mp3Path: string): Promise<{ success: boolean; error?: string }> {
  if (!(await checkFfmpeg())) {
    return { success: false, error: 'FFmpeg is required to convert Gemini TTS audio.' };
  }
  return new Promise((resolve) => {
    const ffmpeg = spawn('ffmpeg', ['-y', '-i', wavPath, '-codec:a', 'libmp3lame', '-b:a', '192k', mp3Path]);
    let errorOutput = '';
    ffmpeg.stderr.on('data', (d: Buffer) => (errorOutput += d.toString()));
...
ipcMain.handle('gemini:speak', async (_, text: string, voiceId: string, apiKey: string, stylePrompt?: string, speed?: number) => {
  try {
    ...
    const tempWav = path.join(app.getPath('temp'), `gemini-tts-${Date.now()}.wav`);
    const tempMp3 = path.join(app.getPath('temp'), `gemini-tts-${Date.now()}.mp3`);
    fs.writeFileSync(tempWav, wavBuffer);

    const convertResult = await wavToMp3(tempWav, tempMp3);
    try { fs.unlinkSync(tempWav); } catch (_e) {}
...
ipcMain.handle('gemini:saveToFile', async (_, text: string, voiceId: string, apiKey: string, outputPath: string, stylePrompt?: string, speed?: number) => {
  try {
    ...
    const pcmBuffer = Buffer.from(part.inlineData.data, 'base64');
    const wavBuffer = pcmToWav(pcmBuffer);
    const tempWav = path.join(app.getPath('temp'), `gemini-temp-save-${Date.now()}.wav`);
    fs.writeFileSync(tempWav, wavBuffer);

    const convertResult = await wavToMp3(tempWav, outputPath);
    try { fs.unlinkSync(tempWav); } catch (_e) {}
    if (!convertResult.success) return { error: convertResult.error };
