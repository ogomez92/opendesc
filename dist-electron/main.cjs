...
const DEFAULT_SUBTITLE_SETTINGS = {
    transcriptionPrompt: 'Transcribe the audio. You can also translate by specifying the desired output language and format in this prompt.',
...
function loadConfig() {
    try {
        if (fs.existsSync(CONFIG_PATH)) {
            ...
            const transcriptionPrompt = parsedSubtitleSettings.transcriptionPrompt ||
                parsedSubtitleSettings.translationPrompt ||
...
function saveConfig(config) {
    try {
        ...
        const subtitleSettings = {
            ...DEFAULT_SUBTITLE_SETTINGS,
            ...parsedSubtitleSettings,
            transcriptionPrompt: parsedSubtitleSettings.transcriptionPrompt ||
                parsedSubtitleSettings.translationPrompt ||
                DEFAULT_SUBTITLE_SETTINGS.transcriptionPrompt,
        };
        const normalized = {
...
async function adjustAudioSpeed(inputPath, outputPath, speed) {
    ...
    return new Promise((resolve) => {
        const ffmpeg = (0, child_process_1.spawn)('ffmpeg', ['-y', '-i', inputPath, '-filter:a', filter, '-vn', tempOutput]);
        let errorOutput = '';
            ...
            if (code === 0) {
                if (inputPath === outputPath) {
                    try {
                        ...
                        resolve({ success: false, error: `Failed to overwrite original file: ${e.message}` });
                    }
                }
                else {
                    resolve({ success: true, path: outputPath });
...
// Azure TTS
electron_1.ipcMain.handle('azure:getVoices', async (_, apiKey, region) => {
    try {
...
electron_1.ipcMain.handle('azure:speak', async (_, text, voiceId, apiKey, region, speed) => {
    try {
        const ssml = `
    ...
    `;
        ...
        const tempPath = path.join(electron_1.app.getPath('temp'), `azure-tts-${Date.now()}.mp3`);
...
electron_1.ipcMain.handle('azure:saveToFile', async (_, text, voiceId, apiKey, region, outputPath, speed) => {
    try {
        const ssml = `
      <speak version="1.0" xmlns="http://www.w3.org/2001/10/synthesis" xml:lang="en-US">
        <voice name="${voiceId}">${text}</voice>
    ...
    `;
        ...
        const audioBuffer = Buffer.from(await response.arrayBuffer());
        if (speed && speed !== 1) {
            const tempPath = path.join(electron_1.app.getPath('temp'), `azure-temp-save-${Date.now()}.mp3`);
            fs.writeFileSync(tempPath, audioBuffer);
...
// ElevenLabs TTS
electron_1.ipcMain.handle('elevenlabs:getVoices', async (_, apiKey) => {
    try {
...
electron_1.ipcMain.handle('elevenlabs:speak', async (_, text, voiceId, apiKey, modelId, speed) => {
    try {
        ...
        const audioBuffer = Buffer.from(await response.arrayBuffer());
        const tempPath = path.join(electron_1.app.getPath('temp'), `elevenlabs-tts-${Date.now()}.mp3`);
        fs.writeFileSync(tempPath, audioBuffer);
        if (speed && speed !== 1) {
            const result = await adjustAudioSpeed(tempPath, tempPath, speed);
            if (!result.success) {
...
electron_1.ipcMain.handle('elevenlabs:saveToFile', async (_, text, voiceId, apiKey, outputPath, modelId, speed) => {
    try {
        ...
        const audioBuffer = Buffer.from(await response.arrayBuffer());
        if (speed && speed !== 1) {
            const tempPath = path.join(electron_1.app.getPath('temp'), `elevenlabs-temp-save-${Date.now()}.mp3`);
            fs.writeFileSync(tempPath, audioBuffer);
            const result = await adjustAudioSpeed(tempPath, outputPath, speed);
            try {
                fs.unlinkSync(tempPath);
...
// Google Cloud TTS
electron_1.ipcMain.handle('google:getVoices', async (_, apiKey) => {
    try {
...
electron_1.ipcMain.handle('google:speak', async (_, text, voiceId, apiKey, speed) => {
    try {
        ...
        const tempPath = path.join(electron_1.app.getPath('temp'), `google-tts-${Date.now()}.mp3`);
        fs.writeFileSync(tempPath, audioBuffer);
        if (speed && speed !== 1) {
            const result = await adjustAudioSpeed(tempPath, tempPath, speed);
            if (!result.success) {
...
electron_1.ipcMain.handle('google:saveToFile', async (_, text, voiceId, apiKey, outputPath, speed) => {
    try {
        const languageCode = voiceId.split('-').slice(0, 2).join('-');
        const response = await fetch(`https://texttospeech.googleapis.com/v1/text:synthesize?key=${apiKey}`, {
            method: 'POST',
        ...
        const audioBuffer = Buffer.from(data.audioContent, 'base64');
        if (speed && speed !== 1) {
            const tempPath = path.join(electron_1.app.getPath('temp'), `google-temp-save-${Date.now()}.mp3`);
            fs.writeFileSync(tempPath, audioBuffer);
            const result = await adjustAudioSpeed(tempPath, outputPath, speed);
            try {
                fs.unlinkSync(tempPath);
...
async function extractAudioSegment(inputPath, startMs, endMs) {
    ...
    const tempDir = fs.mkdtempSync(path.join(electron_1.app.getPath('temp'), 'tts-segment-'));
...
electron_1.ipcMain.handle('gemini:transcribe', async (_, audioPath, apiKey, prompt, startMs, endMs, normalizeAudio = true) => {
    let tempDir;
...
async function extractAudioToWav(inputPath) {
    if (!(await checkFfmpeg())) {
        return { error: 'FFmpeg is required for alignment.' };
    }
    const tempDir = fs.mkdtempSync(path.join(electron_1.app.getPath('temp'), 'align-audio-'));
...
function computeEnergyProfile(wavPath) {
...
function normalizeSeries(values) {
    ...
    for (let i = 0; i < values.length; i++)
        data[i] = (values[i] - mean) / std;
    return { data, mean, std };
}
function findBestOffset(a, b, hopMs) {
...
async function muxAligned(videoPath, audioPath, offsetMs, outputPath) {
    const filters = [];
    if (offsetMs >= 0) {
...
electron_1.ipcMain.handle('alignment:checkEnv', async () => {
    const ffmpegAvailable = await checkFfmpeg();
    return { ffmpegAvailable };
});
electron_1.ipcMain.handle('alignment:run', async (_, options) => {
    const logs = [];
    ...
    const baseDir = providedOutput !== null
        ? path.dirname(providedOutput)
        : fs.mkdtempSync(path.join(electron_1.app.getPath('temp'), 'align-output-'));
    let lastOutput;
    const reportData = [];
    const allOutputs = [];
...
electron_1.ipcMain.handle('dialog:saveAlignOutput', async () => {
    const { dialog } = await Promise.resolve().then(() => __importStar(require('electron')));
    const result = await dialog.showSaveDialog(mainWindow, {
        filters: [
...
electron_1.ipcMain.handle('alignment:saveOutput', async (_, sourcePath, suggestedName) => {
    try {
...
electron_1.ipcMain.handle('dialog:pickAlignPaths', async (_, kind) => {
    const { dialog } = await Promise.resolve().then(() => __importStar(require('electron')));
...
electron_1.ipcMain.handle('dialog:chooseFolder', async () => {
    const { dialog } = await Promise.resolve().then(() => __importStar(require('electron')));
    const result = await dialog.showOpenDialog(mainWindow, {
...
electron_1.ipcMain.handle('dialog:openMediaFile', async () => {
    const { dialog } = await Promise.resolve().then(() => __importStar(require('electron')));
    const result = await dialog.showOpenDialog(mainWindow, {
        properties: ['openFile'],
        filters: [
...
electron_1.ipcMain.handle('dialog:openSrtFile', async () => {
    const { dialog } = await Promise.resolve().then(() => __importStar(require('electron')));
    const result = await dialog.showOpenDialog(mainWindow, {
        properties: ['openFile'],
...
// Play audio file
electron_1.ipcMain.handle('audio:play', async (_, filePath) => {
    if (mainWindow) {
...
async function checkFfmpeg() {
    return new Promise((resolve) => {
        const process = (0, child_process_1.spawn)('ffmpeg', ['-version']);
...
async function getAudioDuration(filePath) {
    return new Promise((resolve, reject) => {
        ...
        const process = (0, child_process_1.spawn)('ffmpeg', ['-i', filePath]);
        let output = '';
...
electron_1.ipcMain.handle('system:checkFfmpeg', async () => {
    return checkFfmpeg();
});
electron_1.ipcMain.handle('system:createTempDir', async () => {
    const tempDir = fs.mkdtempSync(path.join(electron_1.app.getPath('temp'), 'tts-srt-'));
    return tempDir;
});
electron_1.ipcMain.handle('system:removeDir', async (_, dirPath) => {
    try {
...
electron_1.ipcMain.handle('system:getAudioDuration', async (_, targetPath) => {
    try {
...
async function extractAudioFromVideo(inputPath) {
    ...
    const tempDir = fs.mkdtempSync(path.join(electron_1.app.getPath('temp'), 'tts-audio-'));
...
electron_1.ipcMain.handle('subtitle:convertVideoToAudio', async (_, inputPath) => {
    try {
...
async function prepareAudioForGemini(inputPath) {
    if (!(await checkFfmpeg())) {
        return { path: inputPath };
    }
    const tempDir = fs.mkdtempSync(path.join(electron_1.app.getPath('temp'), 'tts-gemini-'));
...
async function normalizeAudioForPlayback(inputPath) {
    if (!(await checkFfmpeg())) {
        return { audioPath: inputPath };
    }
    const tempDir = fs.mkdtempSync(path.join(electron_1.app.getPath('temp'), 'tts-audio-play-'));
...
electron_1.ipcMain.handle('subtitle:prepareAudioForPlayback', async (_, inputPath) => {
    try {
...
electron_1.ipcMain.handle('subtitle:ensureTempSrt', async (_, suggestedName) => {
    const tempDir = fs.mkdtempSync(path.join(electron_1.app.getPath('temp'), 'tts-subs-'));
...
electron_1.ipcMain.handle('subtitle:writeSrt', async (_, targetPath, content) => {
    try {
...
electron_1.ipcMain.handle('subtitle:saveSrt', async (_, content, suggestedName) => {
    const { dialog } = await Promise.resolve().then(() => __importStar(require('electron')));
    const result = await dialog.showSaveDialog(mainWindow, {
...
electron_1.ipcMain.handle('subtitle:translateWithGemini', async (_, text, apiKey, prompt) => {
    try {
        const genAI = new generative_ai_1.GoogleGenerativeAI(apiKey);
        const model = genAI.getGenerativeModel({ model: 'gemini-2.5-flash' });
...
electron_1.ipcMain.handle('system:saveToFile', async (_, text, voiceName, outputPath, speed) => {
    ...
    const tempWavPath = path.join(electron_1.app.getPath('temp'), `tts_sys_raw_${Date.now()}.wav`);
    let saveResult = { error: 'Unsupported platform' };
    if (process.platform === 'win32') {
        try {
            const tempScriptPath = path.join(electron_1.app.getPath('temp'), `sapi_save_${Date.now()}.ps1`);
            const psScript = `
      ...
      `;
            ...
            const ps = (0, child_process_1.spawn)('powershell', ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', tempScriptPath, '-Text', text, '-VoiceName', voiceName, '-OutputPath', tempWavPath]);
            saveResult = await new Promise((resolve) => {
                ...
                ps.on('close', (code) => {
                    ...
                    if (code === 0)
                        ...
                        resolve({ error: `PowerShell error: ${errorOutput}` });
                });
            });
        }
        catch (error) {
        ...
        try {
            ...
            const child = (0, child_process_1.spawn)('say', ['-v', cleanVoiceName, '-o', tempWavPath]);
            ...
            saveResult = await new Promise((resolve) => {
                ...
                child.on('close', (code) => {
                    if (code === 0)
                        ...
                        resolve({ error: `macOS 'say' error: ${errorOutput}` });
                });
    ...
    if (speed && speed !== 1) {
        const speedWavPath = path.join(electron_1.app.getPath('temp'), `tts_sys_speed_${Date.now()}.wav`);
    ...
    if (isMp3) {
        return new Promise((resolve) => {
            const ffmpeg = (0, child_process_1.spawn)('ffmpeg', ['-y', '-i', currentPath, '-codec:a', 'libmp3lame', '-qscale:a', '2', outputPath]);
            ffmpeg.on('close', (code) => {
                try {
                    fs.unlinkSync(currentPath);
                }
...
electron_1.ipcMain.handle('system:mixAudio', async (_, clips, outputPath, backgroundPath) => {
    if (!(await checkFfmpeg())) {
        return { error: 'FFmpeg is required for mixing audio.' };
    }
    const tempDir = fs.mkdtempSync(path.join(electron_1.app.getPath('temp'), 'tts-mix-'));
    ...
    try {
        ...
        const concatResult = await new Promise((resolve) => {
            const ffmpegConcat = (0, child_process_1.spawn)('ffmpeg', [
                '-f',
                'concat',
                '-safe',
                '0',
...
electron_1.ipcMain.handle('subtitle:getConvertCacheDir', async () => {
    try {
...
electron_1.ipcMain.handle('subtitle:readConvertCache', async () => {
    try {
        if (!fs.existsSync(CONVERT_CACHE_DIR)) {
            return { entries: {} };
        }
...
electron_1.ipcMain.handle('subtitle:writeConvertCache', async (_, entries) => {
    try {
        if (!fs.existsSync(CONVERT_CACHE_DIR)) {
            fs.mkdirSync(CONVERT_CACHE_DIR, { recursive: true });
        }
